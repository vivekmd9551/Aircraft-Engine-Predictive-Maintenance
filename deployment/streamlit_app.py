...
.alert-critical {{
background: #FFF1EE;
border: 1.5px solid rgba(240,84,56,0.25);
}}
...
if page == "Home":
    ...
    with col_info:
        ...
        <p style="font-size:0.82rem;color:#6B7280;margin-top:0.2rem;">RMSE 8.96 &middot; R&#178; 0.9528</p>
        </div>
...
# ═══════════════════════════════════════════
    ...
    with c2:
        st.metric("Best MAE", "6.27", delta="Random Forest")
    with c3:
        st.metric("Best R²", "0.9528", delta="LSTM")
    with c4:
        st.metric("vs Target", "−9.04", delta="50% better", delta_color="normal")
...
# ═══════════════════════════════════════════
    ...
    with col_ctrl:
        ...
        c1, c2, c3, c4 = st.columns(4)
        with c1:
            st.metric("Unscheduled Failure", "$500,000", help="Cost per catastrophic failure")
        with c2:
            st.metric("Scheduled Maintenance", "$50,000", help="Preventive maintenance cost")
        with c3:
            st.metric("Year 1 ROI", f"{roi1:.0f}%", delta=f"vs ${dev_cost/1000:.0f}K invest")
        with c4:
            st.metric("Payback Period", f"{payback:.1f} mo", help="Months to break even")
...
# ═══════════════════════════════════════════
    ...
    with col2:
        ...
        BE Computer Science Graduate &middot; Data Science &amp; AI/ML Specialist &middot; Aviation Technology Enthusiast
        </p>
        </div>
        """, unsafe_allow_html=True)
        st.markdown("""
...
NASA C-MAPSS &middot; Streamlit &middot; v2.0 &middot; 2026
</p>
</div>
""", unsafe_allow_html=True)
