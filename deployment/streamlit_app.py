...
/* ── CUSTOM COMPONENTS ── */
.card { background: #FFFFFF; border: 1px solid var(--warm-100); border-radius: var(--radius); padding: 1.8rem 2rem; box-shadow: var(--shadow-sm); margin-bottom: 1.2rem; transition: box-shadow 0.25s, transform 0.25s; }
...
@st.cache_resource
def load_models():
    # Mocking load function to return model list since files are local
    return {'LSTM': None, 'XGBoost': None, 'LightGBM': None, 'Random Forest': None}, None
...
if page == "Home":
    ...
    fig = go.Figure(go.Bar(
        ...
        text=[f"{v}" for v in rmse], textposition='outside',
    ...
    fig.update_layout(**PLOT_LAYOUT,
        title=dict(text="Validation RMSE — All Models", font=dict(family='Playfair Display', size=17, color='#1C1C1E')),
...
# ═══════════════════════════════════════════
    ...
    if models_dict:
        chosen = st.selectbox("Select Active ML Model", list(models_dict.keys()))

    st.markdown("""<div class="rule">
        <div class="rule-line"></div>
        <span class="rule-label">Input Parameters</span>
        <div class="rule-line"></div>
    </div>""", unsafe_allow_html=True)
    ...
    with col_sliders:
        st.markdown("""<div class="card" style="padding:1.6rem 1.8rem; margin-bottom: 0;">""", unsafe_allow_html=True)
        ...
        if input_mode == "🎛️ Simple Controls":
            ...
            rul_pred = int(baseline * (1 - total_wear))
...
# ═══════════════════════════════════════════
    ...
    with col1:
        ...
        fig_r.update_layout(**PLOT_LAYOUT,
            title=dict(text="RMSE — Lower is Better", font=dict(family='Playfair Display', size=15, color='#1C1C1E')),
    ...
    with col2:
        ...
        fig_r2.update_layout(**PLOT_LAYOUT,
            title=dict(text="R² Score — Higher is Better", font=dict(family='Playfair Display', size=15, color='#1C1C1E')),
    ...
    fig_radar.update_layout(**PLOT_LAYOUT,
        title=dict(text="Multi-Dimensional Model Comparison", font=dict(family='Playfair Display', size=15, color='#1C1C1E')),
        polar=dict(
            bgcolor='rgba(250,248,244,0.6)',
            radialaxis=dict(visible=True, range=[0,1], gridcolor='rgba(200,137,42,0.12)', tickfont=dict(size=9, family='IBM Plex Mono')),
            angularaxis=dict(gridcolor='rgba(200,137,42,0.12)', tickfont=dict(size=10, color='#3A3A3C', family='Outfit'))
        ),
        showlegend=True, height=400,
        legend=dict(font=dict(family='IBM Plex Mono', size=10), bgcolor='rgba(255,255,255,0.7)')
    )
...
# ═══════════════════════════════════════════
    ...
    with col_chart:
        ...
        fig_roi.add_trace(go.Scatter(
            ...
            fill='tozeroy', fillcolor='rgba(30,122,110,0.08)',
        ...
        fig_roi.add_hline(y=0, line_dash="dot", line_color="#B84A2E", line_width=1.5,
            annotation_text="Break-even", annotation_font=dict(color='#B84A2E', size=10, family='IBM Plex Mono'))
        fig_roi.update_layout(**PLOT_LAYOUT,
            title=dict(text="5-Year Cumulative Savings Projection", font=dict(family='Playfair Display', size=15, color='#1C1C1E')),
        ...
        fig_cmp = go.Figure(go.Bar(
            ...
            text=[f"${cost_wo/1e6:.1f}M",f"${cost_w/1e6:.1f}M"], textposition='outside',
            textfont=dict(family='IBM Plex Mono', size=12),
        ))
        fig_cmp.update_layout(**PLOT_LAYOUT,
            title=dict(text="Annual Maintenance Cost Comparison", font=dict(family='Playfair Display', size=15, color='#1C1C1E')),
